...
# 选择特征和标签
features = df[['中单净流入', '中单流入净占比', '主力净流入', '主力净流入占比', '大单净流入', '大单流入净占比', '小单净流入', '小单流入净占比', '超大单净流入', '超大单流入净占比', '收盘价']]
labels = df['涨跌幅']
...
# 给出买入或卖出的建议
def get_recommendation(prediction, threshold=0.5):
...
recommendations = [get_recommendation(pred[0]) for pred in test_predict]

# 输出建议
for date, rec in zip(df['日期'][train_size + time_step + 1:], recommendations):
    print(f"日期: {date.date()}, 建议: {rec}")